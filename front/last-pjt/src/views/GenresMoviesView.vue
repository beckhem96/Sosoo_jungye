<template>
  <div class="col-sm-12">
    <div class="not-italic mt-lg text-2xl font-extrabold ml-lg text-violet50"><h1>{{ get_genre() }}</h1></div>
...
export default {
  ...
  methods: {
    ...
    setPages () {
			let numberOfPages = Math.ceil(this.genreMovies.genres_movies.length / this.perPage);
		...
		paginate () {
			...
			let to = (page * perPage);
			return  this.genreMovies.genres_movies.slice(from, to);
		},
  },
  created() {
